...
- **登录**:双端口重定向链认证,验证码支持 ddddocr 自动识别(5 次重试)与手动输入;默认**无账号模式**——不自动登录,用户在设置页手动输入学号/密码/验证码登录
...
| `AUTO_LOGIN_ENABLED` | 服务重启/会话过期后凭保存的凭证自动重新登录教务(**默认关闭**——无账号模式,所有访问者需手动登录) | `False` |
...
- 教务系统需要校园网或 VPN 才能访问;Session 过期后需在设置页重新手动登录(默认不自动登录;设 `AUTO_LOGIN_ENABLED=True` 可恢复凭保存凭证自动登录)。
