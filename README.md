...
- 🎨 **色彩标注** — 不同课程不同颜色，单双周区分
- 📱 **响应式布局** — 大节行高按小节比例自适应，桌面表格 + 手机列表双视图
- 📆 **周次筛选** — 左右箭头切换教学周，只看当前周的课

### 📝 考试安排
- 📋 **考试自动同步** — 考试时间、地点、座位号一键获取
- ⏰ **倒计时显示** — 距每场考试的天数和小时数，临近考试高亮提醒
- 🏫 **考场分布** — 按时间排序，清晰展示考试日程
...
- 📊 **各学期绩点明细** — 卡片式展示每学期 GPA、学分、课程数，点击跳转详情
- 🚫 **通识选修课过滤** — 按 NJUST 评奖/保研规则自动排除通识教育选修课
- 🏆 **四六级成绩** — 从教务拉取 CET-4/CET-6 成绩，自动折算百分制
- 🎓 **保研推免 GPA 模式** — 用 CET 折算分替换英语模块（8 学分），计算保研口径绩点
...
### 🖼️ 校历 & 照片墙
- 📅 **校历查看** — 支持上传校历图片，横向滚动浏览
- 🗺️ **校园地图** — 钟山校区地图等实用图片
- 🔍 **全屏预览** — 点击图片进入 Lightbox 模式，支持键盘 ← → 切换、Esc 关闭
- 🖱️ **滚轮横向滑动** — 鼠标滚轮自动转为水平滚动

### 🔧 系统功能
- 💾 **本地数据库** — SQLite 存储，离线可查历史数据
- 🔄 **一键刷新** — 课表/考试/成绩/评教数据随时从教务更新
- 🏫 **教务直连** — 无需校园网环境，直接登录教务系统
- 🌐 **智慧理工登录** — 支持 SSO 两步验证，兼容多种网络环境
- 📱 **多设备访问** — 手机连同一 WiFi 即可查看，支持 PWA 离线使用
...
电脑 IP 在页面底部显示。也可以添加到主屏幕获得 PWA 全屏体验。
...
| 模式 | 说明 | 适用场景 |
|------|------|----------|
| 📋 奖学金/综测 | 排除通识选修课，按原始成绩计算 | 评奖评优、综测排名 |
| 🎓 保研推免 | CET 折算分替换英语模块（8 学分），其他同上 | 推免/保研绩点预估 |
...
将图片文件放入 `static/gallery/` 目录，刷新校历页面即可看到。
