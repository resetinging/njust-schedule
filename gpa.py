...
def calc_semester_gpas(db) -> list[dict]:
    """计算每个学期的绩点汇总

    参数:
        db: 数据库连接（sqlite3.Connection）

    返回按学期排序的列表: [{semester, gpa, gpa_counted, credits, count}, ...]
    其中 gpa_counted 为排除通识选修课后的计内绩点（评奖/保研口径）
    """
    rows = db.execute(
        """SELECT academic_year, semester, course_name, score, credit, grade_point, course_nature
           FROM grades ORDER BY academic_year DESC, semester DESC"""
    ).fetchall()

    # 按学期分组
    groups = OrderedDict()
    for r in rows:
        key = f"{r['academic_year']}-{r['semester']}"
        ...
        groups[key].append({
            ...
            "course_nature": r["course_nature"] or "",
        })

    result = []
    for sem, grades in groups.items():
        # 补充绩点
        for g in grades:
            if float(g.get("grade_point", 0) or 0) == 0:
                g["grade_point"] = score_to_gp(g.get("score", ""))

        gpa_all = calc_gpa(grades, gpa_only=False)
        gpa_counted = calc_gpa(grades, gpa_only=True)

        counted_grades = [g for g in grades if is_gpa_course(g.get("course_nature", ""))]
        counted_credits = round(sum(
            float(g.get("credit", 0) or 0) for g in counted_grades
        ), 1)
