...
def main():
    ...
    checks = [
        ...
        ('JS data 含弹窗状态', all(k in js for k in ('showLinks', 'linkKeyword', 'linkShown'))),
        ('JS onOpenLinks', 'onOpenLinks()' in js),
        ('JS onLinksClose', 'onLinksClose()' in js),
        ('JS onLinkSearch', 'onLinkSearch(e)' in js),
        ('JS onLinkClear', 'onLinkClear()' in js),
        ...
        ('WXML 搜索绑定', 'onLinkSearch' in wxml),
        ('WXML 清空绑定', 'onLinkClear' in wxml),
        ('WXML 分组渲染', 'linkShown' in wxml),
